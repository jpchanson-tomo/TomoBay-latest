...
import com.tomoparts.tomoBay.External;
/**
...
public final class HttpServer
{
	...
	public void start(int port) throws MalformedURLException
    {
		...
		////////////////////////////////////////////////////////////////////unsecured shit for retards
		Server unsecuredServer = new Server();
//		unsecuredServer.addConnector(HttpServer.httpConnector(unsecuredServer, port));
		ContextHandlerCollection unsecContexts = new ContextHandlerCollection();
		unsecContexts.setHandlers(new Handler[] {HttpServer.setUpUnsecuredServlet()});
		unsecuredServer.setHandler(unsecContexts);
		unsecuredServer.setConnectors(new Connector[] {HttpServer.httpConnector(unsecuredServer, 4321)});
		//////////////////////////////////////////////////////////////////////////////////////////////
        try{server.start();/**server.join();**/unsecuredServer.start();}
	...
	/**
	 * set up the unsecured HTTP connector so that retards can access certain functionality without
	 * having to deal with browser security exceptions due to self signed certificates.
	 ...
	 **/
	private static ServerConnector httpConnector(Server server, int port)
	{
	...
	private static ServerConnector sslConnector(Server server, int port)
	{
		...
		sslContextFactory.setKeyStorePath(External.Files.JETTY_SSL_KEYSTORE.get());
		sslContextFactory.setKeyStorePassword("h4l1but3.14159_+-=");
		sslContextFactory.setKeyManagerPassword("h4l1but3.14159_+-=");
		sslContextFactory.setCertAlias(External.Files.JETTY_SSL_CERT_NAME.get());
    ...
    private static ContextHandler setUpDataServletHandler()
    {
		...
		serv.addServlet(DataServlet.class, External.Files.PUBLIC_API_ENDPOINT.get());
    ...
    private static ContextHandler setUpGuiHandler() throws MalformedURLException
    {
		...
		res.setBaseResource(Resource.newResource(External.Files.WEB_CLIENT_PATH.get()));
		context1.setHandler(res);
		return context1;
    }
    
    private static ContextHandler setUpUnsecuredServlet()
    {
   	 ContextHandler context0 = new ContextHandler();
 		context0.setContextPath("/");
 		ServletContextHandler serv = new ServletContextHandler();
 		serv.addServlet(RetardServlet.class, External.Files.RETARD_API_ENDPOINT.get());
 		context0.setHandler(serv);
 		return context0;
    }
}
