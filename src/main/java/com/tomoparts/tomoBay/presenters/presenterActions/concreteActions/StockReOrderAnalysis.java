...
import com.tomoparts.tomoBay.model.sql.schema.nonDBFields.NonDBFields;
...
public final class StockReOrderAnalysis implements AbstractPresenterAction
{
	...
	public String execute(String data)
	{
		...
		catch (Exception e) {e.printStackTrace(); return "{Error:'data probably not valid'"+e.getMessage()+"}";}
	}
	...
	private List<String[]> processDBdata(List<HeteroFieldContainer> dbData)
	{
		...
		// Hashmap version
		final Map<String, Integer> tmp_result = new HashMap<String, Integer>();
		for(int i = 0 ; i < dbData.size() ; ++i)
		{
			...
			{
				final String partNo = listing.getPartNumber(j);
				final int partQty = listing.getPartQty(j);
				
				if(tmp_result.containsKey(partNo))
				{tmp_result.put(partNo, tmp_result.get(partNo)+(partQty*listingQty));}
				else
				{tmp_result.put(partNo, partQty*listingQty);}
			}
		}
		
		for(Map.Entry<String, Integer> entry : tmp_result.entrySet())
		{result.add(new String[] {entry.getKey(),entry.getValue()+""});}
		
		for(int i = 0 ; i < result.size() ; ++i)
		{System.out.println(result.get(i)[0] + " : " + result.get(i)[1]);}
		
		System.out.println(dbData.size() + " , " + result.size());
